...
#***********************
def GetAction (tV, tState):
    otherV = tV
    tempA = otherV[tState - 1, :]
...
def UpdateV(tempV, tempReward, s1, a1, s2, a2, alpha, tGamma, theE):
    newV = tempV
    TDE = GetTDE(tempV, tempReward, s1 - 1, a1, s2 - 1, a2, tGamma)
    for state in range(23):
        newV[state, a1] += TDE * alpha * theE[state]
    return newV

def GetStateIndex(indices):
    return board[indices[0], indices[1]]
    
def GetTDE(tempV, tempReward, s1, a1, s2, a2, tGamma):
    gamma = tGamma
    TDE = tempReward + gamma*tempV[s2, a2] - tempV[s1, a1]
    return TDE

def PolicyTime(theta, stateInd):
    
    print("hello")

def GetPolicyValues(theta, stateInd)
...
def RunSimulation(nEpisodes, alpha, theLamb):
    ...
    for i in range(nEpisodes):
        ...
        tempReturn = 0
        t = 0
        tempAction = int(GetAction(V, GetStateIndex(theState)))
        while(board[theState[0], theState[1]] != 23):
            nextState, tempReward = GetNextState(tempAction, theState, t, gamma)
            nextAction = int(GetAction(V, GetStateIndex(nextState)))
            eT = UpdateET(eT, gamma, lamb)
            eT[GetStateIndex(theState)-1] += 1
            V = UpdateV(V, tempReward, GetStateIndex(theState), tempAction, GetStateIndex(nextState), nextAction, alpha, gamma, eT)
