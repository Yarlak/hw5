...
for epsilon in epsilons:
    for alpha in alphas:
        for order in orders:
            ...
            theReturns[0, :] = np.asarray(mc.RunSimulation(nEpisodes, alpha, order, epsilon))
            xValues = list(range(len(theReturns)))
            
            for x in range(trials-1):
                theReturns[x+1, :] = np.asarray(mc.RunSimulation(nEpisodes, alpha, order, epsilon))
            ...
            theName = "mountaincar-q-" + str(order) + "-" + str(alpha) + "-" + str(epsilon) + ".csv"
