...
#******************************
def GetNextAction(x, v, theC, theW):
...
def ResetC(order):
    ...
    tempOrder = order + 1
    c = np.zeros((tempOrder**4,4), dtype=int)
    counter = 0
    
    for i in range(tempOrder):
        for j in range(tempOrder):
            for k in range(tempOrder):
                for l in range(tempOrder):
                    c[counter, :] = np.array([i, j, k, l])
                    counter += 1
    c = c * np.pi
...
def RunEpisode(alpha, order, c, w):
    x = -0.5
    v = 0
    endEpisode = False
    totalReward = 0
    ...
    while (endEpisode == False):
        a = GetNextAction(x, v, c, w)
        x, v = GetNextState(x, v, a)
        endEpisode, r = GetReward(x)
        totalReward += r
    
    return totalReward, w
    
def RunSimulation(nEpisodes, alpha, order):
    
    theReturns = []
    c, w = ResetC(order)
    
    for episode in nEpisodes:
        tempReturn, w = RunEpisode(alpha, order, c, w)
